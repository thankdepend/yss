...
const wish = require('../../../reqApi/wish');
const provinceScoreLine = require('../../../reqApi/provinceScoreLine');
...
describe('完善信息',function () {
   ...
   before('登录', async function(){
      ...
      // 获取修改次数
      const gteUserInfo = await wish.getUser({data:{"p":{},"m":""},ts:time,ticket:TICKET}).then(res => res.result.datas.obj);
      ModifyNum = {
   ...
   describe('设置修改次数',async function(){
      ...
      before('平台登录',async function(){
         platFromInfo = await yysLogin.platfrom({loginName:'cbyy01',password:'Csk001'});
   ...
   it('保存用户-新接口',async function(){
      const scoreLineInfo = await provinceScoreLine.getAosAndProfTypeList({
         data:{
            p:{provinceId: basicData.province['湖北'],},m:''},
         ticket:TICKET,
         ts:time
      ...
      const res = await wish.saveUser(saveReq)
      // console.log(params.data);
      const savePamas = res.params.data.p;
      // console.log('保存响应',res.params.data.p);
      const probUserInfo = await wish.getUser({data:{"p":{},"m":""},ts:time,ticket:TICKET}).then(res => res.result.datas.obj);
