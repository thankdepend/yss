...
class Apply {
    ...
    async stuinfoAssert() {
        ...
        // console.log('实际值', stuinfo.result.datas.obj);
        common.isApproximatelyEqualAssert(this, stuinfo);
    }

    /** 上传图片 */
    async uploadAuth(params) {
        let kaoSImg = doc.pre.peolpe.longUrl[common.getRandomNum(0, doc.pre.peolpe.longUrl.length - 1)];
    ...
    async uploadShenFen(params) {
        let shenFenImg = doc.pre.peolpe.longUrl[common.getRandomNum(0, doc.pre.peolpe.longUrl.length - 1)];
    ...
    async uploadProve(params) {
        let proveImg = doc.pre.peolpe.longUrl[common.getRandomNum(0, doc.pre.peolpe.longUrl.length - 1)];
        const res = await service.uploadAuth(Object.assign({
            data: {
                "p": {
                    "ord": "3",
    ...
    async uploadVideo(params) {
        let video = doc.pre.peolpe.longUrl[common.getRandomNum(0, doc.pre.peolpe.longUrl.length - 1)];
        const res = await service.uploadAuth(Object.assign({
            data: {
                "p": {
                    "ord": "4",
...
applyManage.mockExamineeJson = function (params) {
    console.log('mock数据', LOGINDATA.loginName);
