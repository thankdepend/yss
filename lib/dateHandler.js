...
/**
 * getCurrentTime - 获取当前时间之后x小时的精确时间
 * @alias module:dateHandler
 * @param {Number} hours 
 * @return {string} YYYY-MM-DD HH:mm:ss
 */
dateHandler.getCurrentTimeAfter = (hours) => {
...
/**
 * getCurrentTime - 获取当前时间之前x小时的精确时间
 * @alias module:dateHandler
 * @param {Number} hours 
 * @return {string} YYYY-MM-DD HH:mm:ss
 */
dateHandler.getCurrentTimeBefore = (hours) => {
    let timestamp = new Date().getTime(); //当前的时间戳
    timestamp = timestamp - hours * 60 * 60 * 1000;
    return moment(timestamp).format('YYYY-MM-DD HH:mm:ss');
}
...
/**
 * getCurrentTime - 获取当前时间之前x小时的时间
 * @alias module:dateHandler
 * @param {Number} hours 
 * @return {string} YYYY-MM-DD HH:mm:ss
 */
dateHandler.getCurrentBefore = (hours) => {
    let timestamp = new Date().getTime(); //当前的时间戳
    timestamp = timestamp - hours * 60 * 60 * 1000;
...
/**
 * getCurrentTime - 获取当前时间之后x小时的时间
 * @alias module:dateHandler
 * @param {Number} hours 
 * @return {string} YYYY-MM-DD
 */
dateHandler.getCurrentAfter = (hours) => {
    let timestamp = new Date().getTime(); //当前的时间戳
    timestamp = timestamp - hours * 60 * 60 * 1000;
    return moment(timestamp).format('YYYY-MM-DD');
}
...
/**
 * 获取月份差
 * 2020-12-02 09:18:22  return 2021-01-02 09:18:22
 */
dateHandler.getMonthGap = () => {
    const tm = dateHandler.getCurrentTime();
    let tm1;
    if (tm.split('-')[1] == 12) {
        tm1 = tm.split('-')
        // 年份
        tm1[0] = Number(tm.split('-')[0]) + 1;
        // 月份
        tm1[1] = '01';
        // console.log(tm1.join('-'));
        return tm1.join('-');
    } else {
        // 非12月的情况
        tm1 = tm.split('-')
        // 强行处理2月份
        if (tm.split('-')[2] == 28 && tm.split('-')[1] == 2) {
            tm1[1] = '03';
            tm1[2] = '30';
            return tm1.join('-');
        }
        // 处理月末的情况
        else if (tm.split('-')[2] == 30 || 31) {
            tm1[1] = Number(tm.split('-')[1]) + 1;
            tm1[2] = '30';
            return tm1.join('-');
        } else {
            tm1[0] = Number(tm.split('-')[0]) + 1;
