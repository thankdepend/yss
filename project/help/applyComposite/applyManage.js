...
class Apply {
    ...
    async saveProf (profParams, extraParams) {
        ...
        const examProfList = examProfRes.result.datas.list
        this.baoMingMain = examProfList.find(
            prof =>
                prof.yongHuID == extraParams.yongHuID &&
                prof.zhuanYeID == extraParams.zhuanYeID &&
                prof.riChengID == extraParams.riChengID
        )
        return this;
    }
    ...
    async saveAffirm (params) {
        const res = await print.saveAffirm(params)
        return res;
    }
}
