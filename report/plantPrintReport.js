...
class HtmlReport extends Mochawesome {
    ...
    doneFin({ output, exit }) {
        ...
        /** 邮件主题 */
        const subject = `【中台打印${common.getCurrentDate()}${this.envName}环境】自动化用例报告`;
        /** 收件人 */
        const addressee = this.envName != '线上' ? ['陆星欣<luxxhz@dingtalk.com>', '罗琦<jpw7169@dingtalk.com>'] : ['陆星欣<luxxhz@dingtalk.com>', '罗琦<jpw7169@dingtalk.com>'];
        const atMobiles = stats.failures ? [18829897169, 13750850013] : [];
        const reportPath = this.envName == '测试' ? '中台测试打印' : `中台${this.envName}测试打印`;
        ...
        dingtalkRobot.sendMsg(dingtalkAccessToken, {
            msgtype: 'text',
            text: { content: `中台打印${this.envName}环境执行完毕,用例数:${stats.passes + stats.failures},通过数:${stats.passes},失败数:${stats.failures}\n日志链接:http://192.168.0.23:8081/jenkins/job/autotest/job/${reportPath}/result` },
            at: {
                atMobiles: atMobiles,
                isAtAll: false
            }
