...
class HtmlReport extends Mochawesome {
    ...
    doneFin({ output, exit }) {
        ...
        /** 邮件主题 */
        const subject = `【商陆花二代${common.getCurrentDate()}${this.envName}环境】自动化用例报告`;
        /** 收件人 */
        // const addressee = ['曹亚贞<caoyazhen6371@dingtalk.com>', '邱相师<qiuxiangshi@dingtalk.com>', '章茜<zhangxi0586@dingtalk.com>', '陆星欣<luxxhz@dingtalk.com>', '谢星<uoz1890@dingtalk.com>', '徐谢靠<xuxiekao@dingtalk.com>', '叶建兵<yejianbing1303@dingtalk.com>'];
        const addressee = ['徐谢靠<xuxiekao@dingtalk.com>', '叶建兵<yejianbing1303@dingtalk.com>'];

        let reportPath;
        switch (this.envName) {
            case '合库测试':
                reportPath = 'auto_slh2_d5';
                break;
            case '测试':
                reportPath = 'auto_slh2_d2';
                break;
            case '审核':
                reportPath = 'auto_slh2_d1';
                break;
            case '线上':
                reportPath = 'auto_slh2_cg4';
                break;
            default:
                break;
        }
